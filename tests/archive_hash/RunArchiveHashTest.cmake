...
set(_archive_name "topdrawer.tar.gz")
...
elseif(TEST_CASE STREQUAL "no_fallback_after_mismatch_allowed")
  ...
  set(_mismatch_path "${_local_hint}")
  set(_expected_path "${_local_hint}")
elseif(TEST_CASE STREQUAL "hint_precedence")
  set(_local_hint "${TEST_ROOT}/explicit-input.tar.gz")
  file(WRITE "${_local_hint}" "${_good_contents}")
  file(WRITE "${_archive_dir}/${_archive_name}" "${_bad_contents}")
  file(WRITE "${_cache_path}" "${_bad_contents}")
  set(_net_fetch ON)
  set(_expected_path "${_local_hint}")
elseif(TEST_CASE STREQUAL "archive_directory_fallback")
  set(_local_hint "${TEST_ROOT}/missing-hint.tar.gz")
  file(WRITE "${_archive_dir}/${_archive_name}" "${_good_contents}")
  set(_expected_path "${_archive_dir}/${_archive_name}")
elseif(TEST_CASE STREQUAL "cache_no_fallback")
  file(WRITE "${_cache_path}" "${_bad_contents}")
  file(WRITE "${_download_source}" "${_good_contents}")
  set(_net_fetch ON)
  set(_expect_success OFF)
  set(_mismatch_path "${_cache_path}")
elseif(TEST_CASE STREQUAL "offline_cache_strict")
  file(WRITE "${_cache_path}" "${_good_contents}")
  set(_expect_success OFF)
  set(_expected_failure_text "Required file not found")
elseif(TEST_CASE STREQUAL "download_failure_strict")
  set(_net_fetch ON)
  set(_expect_success OFF)
  set(_expect_cache_absent ON)
  set(_expected_failure_text "Failed to download upstream archive" "${_test_url}" "${_cache_path}" "Reason:")
...
if(_expect_success)
  ...
  if(_expect_mismatch_details)
    foreach(_expected_text
        ...
        "${_expected_sha}"
        "${_bad_sha}"
        "TD_ALLOW_UNVERIFIED_ARCHIVES=ON"
  ...
  if(EXISTS "${_result_file}")
    message(FATAL_ERROR "Failed resolution returned a selected path")
  endif()
...
# Mismatched inputs are retained for inspection; no fallback may replace them.
if(DEFINED _mismatch_path)
  if(NOT EXISTS "${_mismatch_path}")
    message(FATAL_ERROR "Mismatched input was removed: ${_mismatch_path}")
  endif()
  file(SHA256 "${_mismatch_path}" _retained_sha)
  if(NOT _retained_sha STREQUAL _bad_sha)
    message(FATAL_ERROR "Mismatched input was replaced")
  endif()
endif()
